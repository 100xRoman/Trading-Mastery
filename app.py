...
if page == "Tools":
    ...
    with t_journal:
        ...
        with st.form("log_trade"):
            ...
            t_type = c1.selectbox("Position Type", ["LONG", "SHORT"])
            t_cap = c1.number_input("Capital Invested ($)", value=100.0)
            t_lev = c2.number_input("Leverage (x)", value=10)
            p_mode = c2.radio("P&L Input Type", ["%", "$"])
            ...
            if st.form_submit_button("Log Position"):
                ...
                st.session_state.history.append({"Type": t_type, "Capital": t_cap, "Lev": t_lev, "P&L $": usd, "P&L %": pct})
        ...
        if st.session_state.history:
            ...
            met1.metric("Total Net P&L", f"${total_net:,.2f}")
            met2.metric("Win Rate", f"{win_rate:.1f}%")
            met3.metric("Profit Factor", f"{profit_factor:.2f}")
            
            st.table(df)
            if st.button("Clear Journal Data"): st.session_state.history = []; st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with t_compound:
        ...
        s_bal = st.number_input("Starting Capital ($)", value=1000.0, key="comp_start")
        n_doubles = st.number_input("How many times to double?", value=5, step=1)
        final_comp = s_bal * (2**n_doubles)
        st.metric("Final Balance", f"${final_comp:,.2f}")
        st.progress(min((0.35**n_doubles), 1.0))
        st.caption(f"Success Probability: {(0.35**n_doubles)*100:.4f}%")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with t_dca:
        ...
        p1 = col_d1.number_input("Order 1 Price", value=60000.0)
        a1 = col_d1.number_input("Order 1 Amount ($)", value=500.0)
        p2 = col_d2.number_input("Order 2 Price", value=55000.0)
        a2 = col_d2.number_input("Order 2 Amount ($)", value=500.0)
        
        if (p1 > 0 and p2 > 0):
            avg_entry = (a1 + a2) / ((a1/p1) + (a2/p2))
            st.metric("Weighted Average Entry", f"${avg_entry:,.2f}")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with t_be:
        ...
        be_entry = st.number_input("Entry Price", value=50000.0)
        fee_rate = st.number_input("Exchange Fee % (One-way)", value=0.06, format="%.3f")
        # Calculation accounts for entry + exit fee
        breakeven = be_entry * (1 + (fee_rate/100) * 2)
        st.metric("Price to Exit for $0 Loss", f"${breakeven:,.2f}")
        st.info(f"You need a move of {((breakeven/be_entry)-1)*100:.3f}% just to cover fees.")
        st.markdown('</div>', unsafe_allow_html=True)

    # 5. POSITION SIZE % TO MARGIN
    with t_pos:
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.markdown('<p class="indicator-title">📏 Margin Converter</p>', unsafe_allow_html=True)
        wallet = st.number_input("Total Wallet Balance ($)", value=1000.0)
        risk_percent = st.slider("Wallet % to Allocate", 1, 100, 10)
        lev_used = st.number_input("Leverage Used (x)", value=10)
        
        margin_required = (wallet * (risk_percent/100)) / lev_used
        st.metric("Required Margin (Bybit Input)", f"${margin_required:,.2f}")
        st.write(f"This position uses **{risk_percent}%** of your buying power.")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with t_stress:
        ...
        curr_p = st.number_input("Current Entry Price ", value=50000.0)
        curr_l = st.slider("Select Leverage to Test", 1, 100, 20)
        
        liq_move = 100 / curr_l
        st.error(f"Liquidation Level: {liq_move:.2f}% move against you.")
        st.warning(f"A 2% price drop results in a **{2 * curr_l}%** account drawdown.")
        st.markdown('</div>', unsafe_allow_html=True)

    # 7. MARKET SENTIMENT
    with t_sentiment:
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.image("https://alternative.me/crypto/fear-and-greed-index.png", caption="Live Fear & Greed Index")
        st.markdown('</div>', unsafe_allow_html=True)
