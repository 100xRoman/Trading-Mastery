...
def get_live_analysis(symbol, timeframe):
    """
    Fetches real-time data from Bybit and calculates the Top 10 indicators.
    """
    try:
        # Initialize Bybit Public API
        exchange = ccxt.bybit()
        bars = exchange.fetch_ohlcv(symbol, timeframe=timeframe, limit=100)
        df = pd.DataFrame(bars, columns=['time', 'open', 'high', 'low', 'close', 'volume'])
        
        # 1-3. MACD (Line, Signal, Histogram)
        ...
        # 6-8. Bollinger Bands (Volatility/Reversal)
        ...
        if last['close'] > last['ema_50']: score += 1 # Bullish Trend
        if last['MACDh_12_26_9'] > 0 and prev['MACDh_12_26_9'] < 0: score += 2 # MACD Bullish Cross
        if last['ADX_14'] > 25: score += 1 # Strong Momentum
        if last['close'] < last['BBL_20_2.0']: score += 2 # Touched Bottom Band
        
        final_score = max(1, min(10, 5 + score))
        bias = "STRONG BUY" if final_score >= 8 else "BUY" if final_score >= 6 else "SELL" if final_score <= 3 else "NEUTRAL"
...
if page == "Tools":
    ...
    with t_bot:
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.markdown('<p class="indicator-title">🤖 AI Top 10 Indicator Bot</p>', unsafe_allow_html=True)
        ...
        if st.button("EXECUTE LIVE BYBIT SCAN", use_container_width=True):
            with st.spinner("Bot is crunching indicators..."):
                score, bias = get_live_analysis(bot_coin, bot_tf)
                if score:
                    st.divider()
                    r1, r2 = st.columns(2)
                    r1.metric("Confidence Score", f"{score}/10")
                    b_color = "#238636" if "BUY" in bias else "#da3633" if "SELL" in bias else "#8b949e"
                    r2.markdown(f"### Signal: <span style='color:{b_color}'>{bias}</span>", unsafe_allow_html=True)
                    st.info(f"Analysis includes: RSI, MACD, EMA, BB, ADX, and OBV on {bot_coin}.")
                else:
                    st.error(f"Scanner Error: {bias}")
        st.markdown('</div>', unsafe_allow_html=True)
